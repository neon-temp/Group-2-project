...
export default function Dashboard() {
  ...
  return (
    <div className="app-shell flex h-screen bg-gray-50 font-sans overflow-hidden">
      {/* Modal */}
      {showAddModal && (
        <AddResourceModal
      ...
      {/* ── Sidebar ── */}
      <aside className={`app-sidebar fixed inset-y-0 left-0 z-40 w-56 flex flex-col bg-white border-r border-gray-200 transform transition-transform duration-200 md:static md:translate-x-0 md:shrink-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="flex items-center justify-between gap-2.5 px-5 py-5 border-b border-gray-100">
      ...
      <main className="app-main flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3 md:hidden">
          ...
          <div className="text-sm font-semibold text-gray-900">
            {activeNav === 'library' ? 'Library' : activeNav === 'categories' ? 'Categories' : activeNav === 'tags' ? 'Tags' : 'Settings'}
          </div>
          <div className="w-10" />
        </div>
        {/* Categories view */}
        ...
        {activeNav === 'library' && !selectedResource && (
          <>
            <header className="flex items-center gap-4 px-6 py-4 bg-white border-b border-gray-200 phone-pad">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5 phone-pad">
              {/* Stat cards */}
              <div className="phone-stack flex gap-4">
                <StatCard
              ...
              <div className="phone-stack flex items-center justify-between">
                  ...
                  {(['all', 'links', 'snippets'] as FilterTab[]).map(tab => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${
                        activeFilter === tab
                          ? 'bg-stash-accent text-white shadow-sm'
                          : 'text-gray-500 hover:text-white hover:bg-stash-accent-hover'
                      }`}
                    >
              ...
              <div className="bg-white rounded-xl border border-gray-200 px-5">
                {resourcesLoading ? (
                  ...
                  <div className={isGridView ? 'phone-grid grid gap-4 sm:grid-cols-2' : 'space-y-0'}>
                    {filtered.map(r => (
